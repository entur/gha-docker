...
|                                <a name="input_cache"></a>[cache](#input_cache)                                 | string  |  false   |       `"max"`        |                                                                                                                Docker layer cache mode - <br>'max' caches all build stages, <br>'min' caches only the final <br>image layers, 'off' disables caching, <br>'registry' caches to/from a registry <br>image (requires cache_ref).                                                                                                                 |
|                          <a name="input_cache_ref"></a>[cache_ref](#input_cache_ref)                           | string  |  false   |                      | Registry image reference to use <br>for cache storage when cache <br>is 'registry', e.g. 'eu.gcr.io/entur-system-1287/myapp:buildcache'. Must <br>resolve to a registry this <br>workflow already authenticates against (eu.gcr.io or europe-west1-docker.pkg.dev for gcp, acrentur001.azurecr.io for az) <br>-- an arbitrary third-party registry <br>has no credentials here. Required <br>when cache is 'registry', ignored <br>otherwise.  |
...
|                                  <a name="input_pull"></a>[pull](#input_pull)                                  | boolean |  false   |       `false`        |                                                                                                                                                                                       Always attempt to pull a <br>newer version of the base <br>image                                                                                                                                                                                         |
